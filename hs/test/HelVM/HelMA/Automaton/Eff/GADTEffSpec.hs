module HelVM.HelMA.Automaton.Eff.GADTEffSpec (spec) where
...
spec = do
  ...
  describe "Test GADT WFilter0" $ do
    let mockEff = execMockEffWithInput "qwerty0uiop" $ interpretGADTEffDebug wFilterIf0
    forM_
      [ ("Test GADT WFilterIf0 with calculateOutput" , calculateOutput , "qwerty\n"       )
      , ("Test GADT WFilterIf0 with calculateLogsWithLevelDebug" , calculateLogsWithLevelDebug , debugLogs)
      ] $ \(name , action , output) ->
      it name $ action mockEff `shouldBe` output
