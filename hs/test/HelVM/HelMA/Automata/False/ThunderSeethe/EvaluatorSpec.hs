module HelVM.HelMA.Automata.False.ThunderSeethe.EvaluatorSpec where
...
--        , ("morphett" </> "reverse" , "HelMA\n")
    ...
    let mock = (ioExecMockIOWithInput input . eval ThunderSeethe) =<< file
    describe fileName $ do
      it "output" $
         (calculateOutput <$> mock) `goldenShouldIO` buildAbsoluteOutFileName ("ThunderSeethe" </> "output" </> fileName)
      it "logged" $
         (calculateLogged <$> mock) `goldenShouldIO` buildAbsoluteOutFileName ("ThunderSeethe" </> "logged" </> fileName)
