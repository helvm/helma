...
runAsRIOResult action = do
  ...
  runAndHandleError $ runLoggingT action (logOutput logFunc)

runAsRIOResult' ∷ (MonadIO m, MonadReader env m, Has env) ⇒ NoLoggingT (SafeT m) a → m (Either Messages a)
runAsRIOResult' action = runAndHandleError $ runNoLoggingT action

runAndHandleError ∷ (MonadIO m, MonadReader env m, Has env) ⇒ SafeT m a → m (Either Messages a)
runAndHandleError action = do
  res <- runExceptT action
  whenLeft_ res logSafeError
  pure res
