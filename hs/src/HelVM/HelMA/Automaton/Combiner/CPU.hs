...
runCFI :: (ALU m ll, Show (Element ll)) => CFInstruction -> CentralProcessingStep ll m
runCFI (Mark      _) = pure
runCFI (Branch  o t) = branchInstruction t o
runCFI (Labeled o i) = labeledInstruction i o
runCFI  Return       = popAddress

popAddress :: ALU m ll => CentralProcessingMemory ll -> m $ CentralProcessingMemory ll
...
branchInstruction :: (ALU m ll, Show (Element ll)) => BranchTest -> BranchOperand -> CentralProcessingStep ll m
...
branchSwappedInstruction :: (ALU m ll, Show (Element ll)) => BranchTest -> CentralProcessingStep ll m
...
branchTopInstruction :: (ALU m ll, Show (Element ll)) => BranchTest -> CentralProcessingStep ll m
...
branchImmediateInstruction :: (ALU m ll, DynamicLabel l) => BranchTest -> l -> CentralProcessingStep ll m
...
branchArtificialInstruction :: ALU m ll => BranchTest -> Label -> CentralProcessingStep ll m
...
branch :: ALU m ll => BranchTest -> Element ll -> m InstructionCounter -> CentralProcessingStep ll m
branch t e icM cpm
...
labeledInstruction :: (ALU m ll, Show (Element ll)) => LabelOperation -> LabelOperand -> CentralProcessingStep ll m
...
labeledTopInstruction :: (ALU m ll, Show (Element ll)) => LabelOperation -> CentralProcessingStep ll m
labeledTopInstruction i cpm = appendError "CPM.labeledTopInstruction" $ uncurry (labeledImmediateInstruction i) =<< cpmPop1 cpm

labeledImmediateInstruction :: (ALU m ll, DynamicLabel l) => LabelOperation -> l -> CentralProcessingStep ll m
...
labeledArtificialInstruction :: ALU m ll => LabelOperation -> Label -> CentralProcessingStep ll m
...
cpmPop1 :: ALU m ll => CentralProcessingMemory ll -> m (Element ll , CentralProcessingMemory ll)
cpmPop1 (CPM cm s) = build <$> pop1 s where
   build (l , s') = (l , CPM cm s')

cpmPop2 :: ALU m ll => CentralProcessingMemory ll -> m (Element ll , Element ll , CentralProcessingMemory ll)
