...
runSLI :: LSU m s r => LSInstruction -> LoadStoreMemory s r -> m $ LoadStoreMemory s r
runSLI Load             = load
runSLI Store            = store
...
load :: LSU m s r => LoadStoreMemory s r -> m $ LoadStoreMemory s r
load (LSM s r) = appendError "LSM.load" $ build =<< pop1 s where
  build (address , s') = loadPure address $ LSM s' r

loadD :: LSU m s r => ImmediateIndex -> LoadStoreMemory s r -> m $ LoadStoreMemory s r
loadD address = loadPure (fromIntegral address)

loadPure :: LSU m s r => Element s -> LoadStoreMemory s r -> m $ LoadStoreMemory s r
loadPure address (LSM s r) = pure $ LSM (push1 (RAM.genericLoad r address) s) r

store :: LSU m s r => LoadStoreMemory s r -> m $ LoadStoreMemory s r
store (LSM s r) = appendError "LSM.store" $ build =<< pop2 s where
  build (value , address , s') = storePure value address $ LSM s' r

storeID :: LSU m s r => Integer -> ImmediateIndex -> LoadStoreMemory s r -> m $ LoadStoreMemory s r
storeID value address = storePure (fromIntegral value) (fromIntegral address)

storePure :: LSU m s r => Element s -> Element s -> LoadStoreMemory s r -> m $ LoadStoreMemory s r
storePure value address (LSM s r) = pure $ LSM s $ RAM.store address value r

moveD :: LSU m s r => ImmediateIndex -> ImmediateIndex -> LoadStoreMemory s r -> m $ LoadStoreMemory s r
moveD src dst lsm@(LSM _ r) = storePure (RAM.genericLoad r src) (fromIntegral dst) lsm

-- | IO

loadOutputChar :: LSU m s r => LoadStoreMemory s r -> m $ LoadStoreMemory s r
...
loadOutputDec :: LSU m s r => LoadStoreMemory s r -> m $ LoadStoreMemory s r
...
storeInputChar :: LSU m s r => LoadStoreMemory s r -> m $ LoadStoreMemory s r
...
storeInputDec :: LSU m s r => LoadStoreMemory s r -> m $ LoadStoreMemory s r
...
-- | Types
type LSU m s r = (ALU m s, RAM.RAM r, Element s ~ Element r)
