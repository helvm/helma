...
import           Prelude                                                hiding (divMod, drop, fromList, splitAt, uncons)
...
indexedInstructionImmediate :: SafeStack m ll => IndexedOperation -> ImmediateIndex -> ll -> m ll
...
-- | Slide instructions
slide :: forall m ll. SafeStack m ll => ImmediateIndex -> ll -> m ll
slide i = appendError "ALU.pop2" . build <.> pop1 where
  build (e , l) = push1 e $ drop (fromIntegral i :: Index ll) l

move :: SafeStack m ll => ImmediateIndex -> ll -> m ll
move i = rollStatic i (i + 1)

rollStatic :: forall m ll. SafeStack m ll => ImmediateIndex -> ImmediateIndex -> ll -> m ll
rollStatic rolls i l = build (olength l) where
  build ll
    | i < 0     = pure l
    | r == 0    = pure l
    ...
    where
      r = rolls `mod` i
      (l2, l1) = splitAt (fromIntegral r :: Index ll) l'
      (l', l3) = splitAt (fromIntegral i :: Index ll) l

-- | Copy instructions
copy :: SafeStack m ll => ImmediateIndex -> ll -> m ll
copy i = teeMap flipPush1 (findSafe $ fromIntegral i)
...
charPush1 :: IntegralStack ll => Char -> ll -> ll
charPush1 = genericPush1 . ord

genericPush1 :: (Integral v , IntegralStack ll) => v -> ll -> ll
...
type Stack ll = (Show ll , IsSequence ll , IndexSafe ll)
