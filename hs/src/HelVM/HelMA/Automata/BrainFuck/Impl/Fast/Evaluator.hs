...
evalSource ∷ (AppSafeEff m , Symbol e) ⇒ Source → FullTape e → DumpType → m ()
evalSource source tape dt = logDump dt =<< flip runList tape =<< parseWithOptimize source

runList ∷ (AppSafeEff m , Symbol e) ⇒ FastInstructionList → FullTape e → m $ Memory e
runList il = nextStep (IM il 0)

nextStep ∷ (AppSafeEff m , Symbol e) ⇒ InstructionMemory → FullTape e → m $ Memory e
nextStep (IM iv ic) = doInstruction (iv `indexMaybe` ic) (IM iv $ ic + 1)

doInstruction ∷ (AppSafeEff m , Symbol e) ⇒ Maybe FastInstruction → InstructionMemory → FullTape e → m $ Memory e
...
doWhile ∷ (AppSafeEff m , Symbol e) ⇒ FastInstructionList → InstructionMemory → FullTape e → m $ Memory e
doWhile _  table tape@(_ , 0:_) = nextStep table tape
doWhile iv table tape           = doWhileWithTape =<< runList iv tape where
  doWhileWithTape ∷ (AppSafeEff m , Symbol e) ⇒ Memory e → m $ Memory e
  doWhileWithTape = doWhile iv table . memoryTape

-- | IO instructions
doOutputChar ∷ (AppSafeEff m , Symbol e) ⇒ InstructionMemory → FullTape e → m $ Memory e
doOutputChar _          (_ ,  []) = error "Illegal State"
doOutputChar table tape@(_ , e:_) = putChar (toChar e) *> nextStep table tape

doInputChar  ∷ (AppSafeEff m , Symbol e) ⇒ InstructionMemory → FullTape e → m $ Memory e
doInputChar table tape = (nextStep table . flip writeSymbol tape) =<< getChar

-- | Terminate instruction
doEnd ∷ AppSafeEff m ⇒ InstructionMemory → FullTape e → m $ Memory e
...
data InstructionMemory
  = IM !FastInstructionList !InstructionCounter
  deriving stock (Eq, Show)
