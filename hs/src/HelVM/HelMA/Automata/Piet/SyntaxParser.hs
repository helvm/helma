...
parseFilledImageWithSplit ∷ MonadSafe m ⇒ (Grid Int, BlockTable) → Grid Color → m (Maybe SyntaxGraph)
parseFilledImageWithSplit (indicesGrid, positionTable) imageGrid =
  let combinedCells = V.zipWith Codel (cells imageGrid) (cells indicesGrid)
      codelGrid     = imageGrid { cells = combinedCells }
   in parseFilledImage (codelGrid, positionTable)

parseFilledImage ∷ MonadSafe m ⇒ (Grid Codel, BlockTable) → m (Maybe SyntaxGraph)
...
searchInitialBlock image = processInitial image =<< justOrThrow "EmptyBlockTableError" (image &! (0, 0))
...
getCurColor image (pos : _)    = extractChromatic =<< fetchNextCodel image pos
...
fetchNextCodel image pos = image &! pos
