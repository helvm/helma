...
runRio ∷ Has env ⇒ ImplType → Maybe AdditionalColorStrategy →  Maybe MulticoloredCodelStrategy → Maybe Natural → Maybe LexerType → RIO.RIO env ()
runRio implType _ _  codelInfo _  = runWithOptions  =<< optionsRio where
